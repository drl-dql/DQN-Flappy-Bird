...
def parse_args():
    ...
    parser.add_argument('-l', '--logs_path', dest='logs_path',
                        help='path of the checkpoint folder', default='./logs', type=str)
...
def main():
    ...
    print(np.array(episode).reshape(-1, 1).repeat(3, 1).shape)
    print(np.column_stack([reward, avg_reward, loss]).shape)

    viz = Visdom(env='main')
    viz.line(
        X=np.array(episode).reshape(-1, 1).repeat(3, 1),
